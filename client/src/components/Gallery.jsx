...
import { getPosts, resetPosts } from "../redux/actions/post";
...
const Gallery = () => {
  ...
  const hasData = postState.hasData;
  ...
  const fetchData = () => {
    setLoading(true);
    dispatch(getPosts({ limit, page, search })).then(() => {
      setLoading(false);
      if (!hasData) {
        setDone(true); // If there is no data on the first page, set done to true
      }
    });
  };
  useEffect(() => {
